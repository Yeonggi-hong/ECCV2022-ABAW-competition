...
def save_plt(model_name, model_info) :
    ...
    #loss_ax.plot(model_info['TrlLoss'], 'y', label='train loss')
    loss_ax.plot(model_info['ValLoss'], 'r', label='val loss')
    loss_ax.set_xlabel('epoch')
    loss_ax.set_ylabel('loss')
    loss_ax.legend(loc='upper left')

    #acc_ax.plot(model_info['train_f1'], 'b', label='train f1')
    acc_ax.plot(model_info['F1'], 'g', label='val f1')
